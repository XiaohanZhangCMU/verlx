...
try:
    import mlflow.system_metrics.metrics.rocm_monitor as rocm_monitor
    rocm_monitor.rocml = None  # Prevents `rocml.smi_shutdown()` from running
except ImportError:
    ...
    def post_close(self):
        ...
        if self._enabled:
            ...
            if self._run_id:
                ...
                try:
                    run_status = self._mlflow_client.get_run(self._run_id).info.status
                    if run_status == 'RUNNING':
                        self._mlflow_client.set_terminated(self._run_id, status='FINISHED')
                except Exception as e:
                    log.warning(f"Unable to terminate MLflow run: {e}")
